...
Runtime state files for MEA. These are created and managed by the system — you should not edit them manually.

## Files

| File | Created by | Purpose |
|---|---|---|
| `mea-config.json` | `/configure-mlea` | Your personal configuration (accounts, rules, tracker) |
| `mea-state.json` | `/scan-now`, daily maintenance | Runtime state: last scan timestamps, stats |
...
`mea-config.template.json` is a starter template showing all config fields. Run `/configure-mlea` to generate the real version with your settings.
