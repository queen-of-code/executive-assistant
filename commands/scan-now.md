...
## What this does
1. Reads `task-data/mlea-config.json` to get mailbox list and classification rules
2. For each mailbox:
   a. Reads `lastScanTimestamp` from `task-data/mlea-state.json`
...
- `task-data/mlea-state.json` is updated after all batches complete
