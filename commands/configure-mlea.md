...
5. Writes `task-data/mlea-config.json`
...
```
MLEA is configured. Now set up 3 scheduled tasks in Cowork:

1. Email Scan — Cadence: Daily (or Hourly if you want more frequent scanning)
   Prompt: "Run the MLEA email scan using the email-scanner skill."

2. Daily Maintenance — Cadence: Daily
   Prompt: "Run the MLEA daily maintenance task."

3. Daily Briefing — Cadence: On weekdays (optional, or run /my-day manually)
   Prompt: "Run the MLEA daily briefing using /my-day."
...
> ⚠️ **State persistence:** MLEA's `task-data/` files must survive between Cowork sessions. This works if MLEA is run inside a **Cowork Project** pointed at the repo directory (projects have persistent local storage). If run as a standalone session, files written inside Cowork's VM sandbox may not persist. **Recommend setting up MLEA inside a Cowork Project.**
